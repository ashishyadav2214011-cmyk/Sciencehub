...
const APP_VERSION = "V93.1-Master-Integrated";
...
const BUILD_ID = "SCIENCEHUB-V93.1-MASTER-INTEGRATED";
...
function fresh(){return {
  schemaVersion:16, appVersion:APP_VERSION,
...
function normalise(raw){
  ...
  out.schemaVersion=Math.max(Number(out.schemaVersion||0),16);
...
function syllabusSnapshot(){
 const counts={}; let sections=0,topics=0;
 for(const cls of [11,12]){
  counts[cls]={};
  const group=syllabusData?.subjects?.[String(cls)]||{};
  for(const subject of subjects){const entry=group[subject]; if(!entry){counts[cls][subject]={sections:0,topics:0};continue;} const sec=(entry.units||[]).length; const top=(entry.units||[]).reduce((n,u)=>n+((u[2]||[]).length),0); counts[cls][subject]={sections:sec,topics:top}; sections+=sec; topics+=top;}
 }
 return {counts,sections,topics};
}
function integrationSnapshot(){
 const sy=syllabusSnapshot();
 return {sections:sy.sections,topics:sy.topics,chapters:db.chapters.filter(c=>c.source==='UPMSP-2026-27').length,linkedTopics:db.topics.filter(t=>t.source==='UPMSP-2026-27').length,tasks:db.tasks.length,questions:db.questions.length,mistakes:db.mistakes.length,revision:db.revision.length,minutes:Number(db.minutes||0),goals:db.goals.length};
}
...
function nextAction(){
 ...
 if(!candidates.length){
   const firstTopic=db.topics.find(t=>t.source==='UPMSP-2026-27');
   if(firstTopic)candidates.push({score:35,title:`Start ${firstTopic.name}`,why:`First unstarted syllabus topic in ${firstTopic.subject}`,action:`go('subject:${firstTopic.subject}')`});
 }
...
function home(){
 const action=nextAction(), done=db.tasks.filter(t=>t.done).length, due=dueRevisions().length, task=openTasks()[0], it=integrationSnapshot(), snap=intelligenceSnapshot();
 const syllabusReady=it.topics>0;
 return `<section class="page home-page home-v5">
  <section class="home-hero-real">
   ...
   <div class="home-hero-copy">
    <div class="eyebrow">SCIENCEHUB • PERSONAL STUDY OS</div>
    <h1>Good evening, Ashu.Ayansh.</h1>
    <p>Understand → Practice → Measure → Improve → Execute</p>
    <div class="hero-pills"><span>Class 11 Core</span><span>UPMSP 2026–27</span><span>Local-first</span></div>
   </div>
  </section>
  ...
  <div class="mission-core">
   ...
   <h2>${esc(task?.title||action.title||"Start one clear study target")}</h2>
   <p>${task?`${esc(task.priority||"normal")} • ${task.minutes||0} min`:esc(action.why)}</p>
   <button class="btn mission-btn" onclick="${task?`startTask('${task.id}')`:action.action}">${task?'START MISSION':'START NEXT ACTION'} <span>→</span></button>
  </div>
  ...
  <section class="card section integration-card"><div class="eyebrow">🔗 CONNECTED STUDY OS</div><h2>One system, one topic identity</h2><p class="muted">${syllabusReady?'UPMSP syllabus is the academic source of truth. Learning, practice, mistakes, revision, progress and time all build from the same study context.':'Load the syllabus to activate the connected topic system.'}</p><div class="integration-grid"><div><b>${it.topics}</b><span>Syllabus topics</span></div><div><b>${it.sections}</b><span>Sections</span></div><div><b>${it.questions}</b><span>Questions</span></div><div><b>${it.mistakes}</b><span>Mistakes</span></div><div><b>${it.revision}</b><span>Revision records</span></div><div><b>${it.minutes}</b><span>Study minutes</span></div></div><div class="integration-flow"><span>Syllabus</span><i>→</i><span>Learn</span><i>→</i><span>Practice</span><i>→</i><span>Repair</span><i>→</i><span>Revise</span><i>→</i><span>Measure</span></div></section>
  <div class="home-section-heading"><div><div class="eyebrow">EXPLORE YOUR UNIVERSE</div><h2>Quick Access</h2></div><span>Core study systems</span></div>
  ...
  <div class="card section ai-center home-ai"><div class="eyebrow">AI COMMAND CENTER</div><h2>Choose your intelligence</h2><div class="ai-grid"><button onclick="aiRole('KuroVen')"><b>🖤 KuroVen</b><span>Action → execution</span></button><button onclick="aiRole('Hikaitage')"><b>🧭 Hikaitage</b><span>Learning + strategy</span></button><button onclick="aiRole('HukoVaige')"><b>🧠 HukoVaige</b><span>Patterns + reflection</span></button><button onclick="go('world')"><b>🌍 WORLD / Aui</b><span>Current-world knowledge when called</span></button></div></div>
  <div class="card section sukoon-home-card"><div class="row"><div><div class="eyebrow">🤍 SUKOON.BRAIN</div><h2>Understanding layer</h2><p class="muted">Listen → Reflect → Analyze → Act. Your chat, icon and settings stay inside ScienceHub.</p></div><img src="./assets/companion/sukoon-brain-icon.png" class="sukoon-home-icon" alt="Sukoon.Brain icon"></div><div class="actions"><button class="btn" onclick="openSukoon()">Open Sukoon Chat</button><button class="btn secondary" onclick="openSukoonBotSettings()">⚙ Chat Settings</button></div></div>
  <div class="home-quote"><span>✦</span><div><small>YOUR NEXT THOUGHT</small><b>${esc(sukoonQuote())}</b></div></div>
  ${sukoonCompanionMarkup()}
  <section class="card section home-intelligence-compact"><div class="eyebrow">🧠 PERSONAL INTELLIGENCE</div><h2>Patterns you can use</h2><p class="muted">Open tasks: ${snap.openCount} • Due revision: ${snap.dueCount} • Weak areas: ${snap.weakCount} • Repeated mistake patterns: ${snap.repeatedMistakes.length}</p><button class="btn secondary" onclick="go('progress')">Open Progress & Intelligence →</button></section>
...
function step68Markup(){const action=nextAction();return `<section class="sh68-panel"><div class="sh68-head"><div><span class="sh68-kicker">SCIENCEHUB INTELLIGENCE</span><h2>Intelligence & Future Center</h2><p>Priority → Track → Verify → Learn</p></div><button onclick="render()">Refresh</button></div><div class="sh68-grid"><article><b>Next Best Action</b><div>${esc(action.title)}</div></article><article><b>Open Tasks</b><div>${openTasks().length}</div></article><article><b>Due Revision</b><div>${dueRevisions().length}</div></article><article><b>Goals</b><div>${db.goals.filter(g=>!g.done).length}</div></article><article><b>Exams</b><div>${db.examTracker.length}</div></article><article><b>PCB Saves</b><div>${db.opportunities.length}</div></article><article><b>World Notes</b><div>${db.worldKnowledge.length}</div></article></div><div class="sh68-actions"><button onclick="addGoal()">+ Goal</button><button onclick="addExam()">+ Exam</button><button onclick="addOpportunity('PCB')">+ PCB Opportunity</button><button onclick="addWorldNote()">+ World Note</button><button onclick="go('world')">🌍 World</button><button onclick="go('exam')">🧭 Exam Tracker</button></div></section>`}
...
if('serviceWorker' in navigator){window.addEventListener('load',async()=>{try{const regs=await navigator.serviceWorker.getRegistrations();for(const r of regs){if(r.scope.includes(location.origin))await r.unregister();}if(window.caches){const keys=await caches.keys();await Promise.all(keys.filter(k=>/^sciencehub-/i.test(k)).map(k=>caches.delete(k)));}const r=await navigator.serviceWorker.register('./sw.js?v=93',{updateViaCache:'none'});await r.update();}catch(e){}})}
